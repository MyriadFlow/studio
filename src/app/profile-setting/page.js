...
function ProfileSettingsPage() {
  ...
  const handleSave = async () => {
    const profileData = {
      ...
      username: userName,
      bio: bio,
      ...(verifiedAccounts.website && { website }),
      ...(verifiedAccounts.twitter && { x }),
      ...(verifiedAccounts.instagram && { instagram }),
  ...
  const verifyWebsite = async (url) => {
    try {
      ...
      // Check if the website has the verification meta tag
      const response = await fetch(`${url}/.well-known/verification.txt`);
      if (response.ok && (await response.text()) === verificationCode) {
        setVerifiedAccounts((prev) => ({ ...prev, website: true }));
        return true;
      }
      return false;
    } catch (error) {
  ...
  const verifyTwitter = async () => {
    try {
      ...
      // Remove @ symbol if present and any URL components
      const twitterHandle = x.replace("@", "").split("/").pop();

      // Ask user to verify ownership
      const verificationCode = Math.random().toString(36).substring(7);

      alert(
        `To verify your Twitter account:\n\n` +
          `1. Add this code to your Twitter bio: ${verificationCode}\n` +
          `2. Click OK once you've updated your bio\n` +
          `3. We'll verify the code in your bio`
      );

      // Here you would typically make an API call to verify the Twitter bio
      // For now, we'll simulate verification with user confirmation
      const confirmed = window.confirm(
        "Did you add the verification code to your Twitter bio?"
      );
      if (confirmed) {
        setVerifiedAccounts((prev) => ({ ...prev, twitter: true }));
        alert("Twitter account verified successfully!");
        return true;
      }

      alert("Verification cancelled or failed. Please try again.");
      return false;
    } catch (error) {
  ...
  const verifyInstagram = async () => {
    try {
      ...
      // The actual verification will happen in the OAuth callback
      return true;
    } catch (error) {
            ...
            <>
              ...
              <div
                style={{
                  ...
                  backgroundImage: coverImage
                    ? `url(${process.env.NEXT_PUBLIC_GATEWAY_URL}/ipfs/${coverImage})`
                    : "none",
              ...
              <div style={{ marginTop: "100px" }}>
                ...
                >
                  ...
                  />
                  <button
                    onClick={() => verifyWebsite(website)}
                    style={{
                      backgroundColor: verifiedAccounts.website
                        ? "#4CAF50"
                        : "#7D4AB5",
                      color: "#ffffff",
                      padding: "10px 20px",
                  ...
                  >
                    {verifiedAccounts.website ? "Verified ✓" : "Verify"}
                  </button>
                </div>
                ...
                >
                  ...
                  <button
                    ...
                    style={{
                      backgroundColor: verifiedAccounts.twitter
                        ? "#4CAF50"
                        : "#7D4AB5",
                      color: "#ffffff",
                      padding: "10px 20px",
                ...
                >
                  ...
                  <button
                    ...
                    style={{
                      backgroundColor: verifiedAccounts.instagram
                        ? "#4CAF50"
                        : "#7D4AB5",
                      color: "#ffffff",
                      padding: "10px 20px",
